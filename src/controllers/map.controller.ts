...
class MapController implements MapControllerInterface {
  ...
  public getSearchHosts(pos: object): Promise<Hosts> {
    let query = `
    SELECT hostName , hostAddress, hostTel, hostPostalCode, hostIdx, hostLatitude, hostLongitude, ( 6371 * acos( cos( radians( ${pos["latitude"]} ) ) * cos( radians( hostLatitude ) ) * cos( radians( hostLongitude ) - radians( ${pos["longitude"]} ) ) + sin( radians( ${pos["latitude"]} ) ) * sin( radians( hostLatitude ) ) ) ) AS distance FROM Host HAVING distance < 10 ORDER BY  distance  DESC LIMIT 0 , 3
    `;
